...
def format_body(body):
    temp = body
    special_characters = ["newline", "new line", "capital", "kama", "fullstop", "full stop","Kama"]
    for character in special_characters:
        pos = temp.find(character)  # Use 'temp' here, not 'body'
        while pos != -1:
            if character == 'newline' or character == 'new line':
                temp = temp.replace(character, '\n')
            elif character == 'fullstop' or character == 'full stop':
                temp = temp.replace(character, '.')
            elif character == 'kama' or character == 'Kama':
                temp = temp.replace(character, ',')
            elif character == 'capital':
                index = temp.find("capital")+8
                temp = temp[:index] + temp[index].upper() + temp[index+1:]
                temp = temp.replace("capital",'', 1)
            pos = temp.find(character)
    temp=temp.replace(' ,',',')
    temp=temp.replace(' .','.')
    temp=temp.replace('  ',' ')
    ...
    if request.method == 'POST':
        ...
        while (flag):
            ...
            if subject == 'N':
                ...
                texttospeech("You meant"+subject+"Say yes to confirm or no to enter again",file+i)
                say=speechtotext(5)
                if say == 'yes' or say == 'N':
                    flag=False
        msg['Subject'] = subject
        ...
        while flag:
            ...
            texttospeech("Say newline if you want to start newline,say capital to make letter capital,say kama to add kama say fullstop to add fullstop",file+i)
            i = i + str(1)
            ...
            if body == 'N':
                ...
                texttospeech("You meant"+body+"Say yes to confirm or no to enter again",file+i)
                say=speechtotext(5)
                if say== 'yes' or say == 'N':
                    flag = False
        body=format_body(body)
